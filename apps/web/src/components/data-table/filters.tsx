...
function getFilterComponentEntries(filters: FiltersConfig): [string, ComponentType<DataTableFilterProps>][] {
  return Object.entries(filters).filter(
    (entry): entry is [string, ComponentType<DataTableFilterProps>] => typeof entry[1] === 'function',
  )
}
...
export function Filters({ config, onFiltersChange, onFilterPageReset }: FiltersProps) {
  ...
  const filterEntries = useMemo(() => getFilterComponentEntries(config), [config])
  ...
  return (
    <Popover open={filterPopoverOpen} onOpenChange={handleFilterPopoverOpenChange}>
      <div className="flex max-w-full flex-wrap items-center justify-start gap-1 rounded-md border border-border/60 px-1 py-0.5">
      ...
      >
        ...
        <div className="flex max-h-[min(70vh,24rem)] min-h-0 flex-1 flex-col gap-3 overflow-y-auto px-4 pt-3 pb-4">
          {filterEntries.map(([key, FilterComponent]) => (
            <FilterComponent
              key={key}
